...
class BiLSTM_CRF(object):
    """
    :param vocab word2id 的字典

    :param word2id 字的placeholder
    :param labels 标签的placeholder
    :param sequence_lengths 句子长度的placeholder

    :param word_embeddings 处理好的词向量

    :param logits 未归一化的预测概率 作为CRF层的输入

    :param loss lose function

    :param train_op 训练函数,减小loss
    ...
    def biLSTM_layer_op(self):
        ...
        with tf.variable_scope("proj"):
            W = tf.get_variable(name="W",
                                ...
                                initializer=tf.contrib.layers.xavier_initializer(),  # 一种经典的权值矩阵的初始化方式
                                dtype=tf.float32)

            b = tf.get_variable(name="b",
                                shape=[self.num_tags],
                                initializer=tf.zeros_initializer(),  # 初始化bias
                                dtype=tf.float32)
            ...
            self.logits = tf.reshape(pred, [-1, s[1], self.num_tags])  # 未归一化的预测概率
    ...
    def demo_one(self, sess, sent):
        ...
        tag = [label2tag[label] for label in label_list[0]]  # 将预测出来的数字序列转化为实际标注序列
        return tag
    ...
    def evaluate_local(self, sess, test_data, raw_data):
        ...
        marked_pre, marked_right, marked_true = 0, 0, 0
